...
- `ADMIN_LOG_TOKEN` - длинный случайный пароль для закрытого журнала диалогов.

## Закрытый журнал диалогов

- просмотр: `/admin/dialogs`;
- выгрузка CSV: `/admin/dialogs.csv`;
- имя пользователя при входе: любое, например `admin`;
- пароль: значение переменной `ADMIN_LOG_TOKEN`;
- файл хранения: `/data/assistant-dialogs.jsonl`;
- в панели отображаются записи за последние 90 дней;
- IP-адреса пользователей не сохраняются.

Постоянное хранилище `/data` подключено через `persistenceMount` в конфигурации Amvera.
